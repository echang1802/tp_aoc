...
# Variables de validadcion de produccion tercerizada
var w[T] binary;

# Variables de quiebre de stock
var z[S] binary;
...
subto demand: forall <t, s> in T*S:
    sum <i> in T with i <= t: (x[i] + y[i] - mu[s,i]) >= -500 * z[s];

# Restriccion de tercerizacion
subto outsource: sum <t> in T: w[t] <= 1;

# Restriccion de quiebre de stock
subto stock: sum <s> in S: z[s] <= 2;
